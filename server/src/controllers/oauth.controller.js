...
const handleGithubCallback = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!userResponse.ok) throw new AppError(502, 'Failed to fetch user profile from GitHub');
    ...
    if (!emailsResponse.ok) throw new AppError(502, 'Failed to fetch user emails from GitHub');
    ...
    if (Array.isArray(emails)) {
      primaryEmail =
        emails.find((email) => email.primary && email.verified)?.email || emails[0]?.email;
    } else {
    ...
    const { accessToken: newAccessToken, refreshToken } = await generateAccessAndRefereshTokens(
      user._id
    );
...
export { handleGithubCallback, getGithubAuthUrl };
